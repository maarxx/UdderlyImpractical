...
This mod will stop your pawns from drinking the milk.

Milk is valuable, it can be used instead of meat when making kibble, but when it is RawTasty, your pawns will drink it straight before it can be used for kibble.

The mod overrides the ThingDef for Milk, changing it's preferability from "RawTasty" to "DesperateOnly".
...
Unzip it, and it will spew out a single folder, which is probably named something like `UdderlyImpractical-master`.
...
`C:\Program Files (x86)\Steam\steamapps\common\RimWorld\Mods\UdderlyImpractical-master\Assemblies`
...
`C:\Program Files (x86)\Steam\steamapps\common\RimWorld\Mods\UdderlyImpractical-master`
...
To do this, just click the button where it says "Branch: master", and then click the option for "beta". Congratulations! You've changed branches! Follow the same steps to download and install, except instead of `UdderlyImpractical-master` it will now be `UdderlyImpractical-beta`. You can have both versions installed, but please don't try to have both versions enabled at once using the in-game Mod menu.
